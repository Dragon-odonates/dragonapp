...
# Read data ---------------------------------------------------------------
df <- readRDS(here::here("data", "occ_50.rds"))

# use EEA 50km grid
# grid <- vect(here("data", "EEA_50km.gpkg"))
# gd_rast <- rast(grid, vals = grid$OBJECTID, res = 50000)
# plot(gd_rast)

# # from transform vect.R
# spdf <- readRDS(here::here("app_vect50", "data", "occ_50.rds"))
# plot(project(spdf, "EPSG:3035"))
# gd <- rasterize(project(spdf, "EPSG:3035"))
# rast(grid, res = 50000)

# statrast <- readRDS(here(
#         "data",
#         "derived-data",
#         paste0("stat_", res_grid, ".rds")
# ))
# statrast <- unwrap(statrast)
# grid10 <- readRDS(here::here(
#         "data",
#         "derived-data",
...
for (s in names(n)) {
        ...
        p_grid <- tapply(df$observationID[ms], df$grid10kmID[ms], nodup) /
                nobs *
                100
...
saveRDS(
        spe_grid,
        here::here("analyses", "app", "data", "sp10000.rds"),
        compress = "xz"
)
saveRDS(
        obs_grid,
        here::here("analyses", "app", "data", "po10000.rds"),
        compress = "xz"
)
saveRDS(
        year_grid,
        here::here("analyses", "app", "data", "py10000.rds"),
        compress = "xz"
)
